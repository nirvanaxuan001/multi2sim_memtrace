...
char *mmu_report_file_name = "/home/kami/data/mmu_report.out";
...
{
	struct mmu_page_t * page_t = mmu->page_hash_table[0];
	unsigned int vtl_addr = 0;
	while (page_t)
	{       vtl_addr++;
		if (page_t->phy_addr == phy_addr )
			{
			vtl_addr = page_t->vtl_addr;
			break;
			}
		page_t = page_t->next;
	}
